...
export const Input = forwardRef<HTMLInputElement, Props>(({ label, hint, error, leftIcon, rightSlot, className, id, ...rest }, ref) => {
    ...
    return (
        <div className="space-y-1.5">
            ...
            <div className="relative">
                {leftIcon && <span className="pointer-events-none absolute inset-y-0 left-3 flex items-center text-fg-muted">{leftIcon}</span>}
                <input
                    id={inputId}
                    ref={ref}
...
export const Textarea = forwardRef<HTMLTextAreaElement, TAProps>(({ label, hint, error, className, id, ...rest }, ref) => {
    const inputId = id ?? rest.name
    return (
        <div className="space-y-1.5">
            {label && (
