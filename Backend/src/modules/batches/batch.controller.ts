...
export const get = async (req: Request, res: Response): Promise<void> => {
    if (!req.auth) return
    const batch = await service.getBatch(req.auth.tenantId, req.params.id as string)
...
export const update = async (req: Request, res: Response): Promise<void> => {
    if (!req.auth) return
    const batch = await service.updateBatch(req.auth.tenantId, req.params.id as string, req.body)
    httpResponse(req, res, 200, responseMessage.SUCCESS, batch)
}

export const remove = async (req: Request, res: Response): Promise<void> => {
    if (!req.auth) return
    await service.deleteBatch(req.auth.tenantId, req.params.id as string)
    httpResponse(req, res, 200, responseMessage.SUCCESS)
}

export const assignStudents = async (req: Request, res: Response): Promise<void> => {
    if (!req.auth) return
    const result = await service.assignStudents(req.auth.tenantId, req.params.id as string, req.body)
...
export const transferStudent = async (req: Request, res: Response): Promise<void> => {
    if (!req.auth) return
    const result = await service.transferStudent(req.auth.tenantId, req.params.id as string, req.body)
