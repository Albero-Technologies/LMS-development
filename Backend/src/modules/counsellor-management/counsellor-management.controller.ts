...
import { type TReportRange } from './counsellor-management.schema'
...
export const myReport = async (req: Request, res: Response): Promise<void> => {
    if (!req.auth) return
    const report = await service.getCounsellorReport(req.auth.tenantId, req.auth.role, req.auth.userId, req.auth.userId, rangeFromQuery(req))
...
export const counsellorReport = async (req: Request, res: Response): Promise<void> => {
    if (!req.auth) return
    const report = await service.getCounsellorReport(req.auth.tenantId, req.auth.role, req.auth.userId, req.params.counsellorId, rangeFromQuery(req))
    httpResponse(req, res, 200, responseMessage.SUCCESS, report)
}
...
export const createTask = async (req: Request, res: Response): Promise<void> => {
    ...
    await writeAudit({ action: 'counsellor.task.create', entityType: 'CounsellorTask', entityId: task.id }, req)
...
export const updateTask = async (req: Request, res: Response): Promise<void> => {
    if (!req.auth) return
    const task = await service.updateTask(req.auth.tenantId, req.auth.role, req.auth.userId, req.params.id, req.body)
    await writeAudit({ action: 'counsellor.task.update', entityType: 'CounsellorTask', entityId: task.id, metadata: { status: task.status } }, req)
    httpResponse(req, res, 200, responseMessage.SUCCESS, task)
}

export const deleteTask = async (req: Request, res: Response): Promise<void> => {
    if (!req.auth) return
    await service.deleteTask(req.auth.tenantId, req.auth.role, req.auth.userId, req.params.id)
    await writeAudit({ action: 'counsellor.task.delete', entityType: 'CounsellorTask', entityId: req.params.id }, req)
