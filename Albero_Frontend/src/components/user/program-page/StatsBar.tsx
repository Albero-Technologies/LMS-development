...
export const StatsBar = ({ stats, tone = 'deep' }: Props) => {
    const bg = tone === 'soft' ? 'var(--section-soft)' : tone === 'gradient' ? 'var(--gradient-aurora)' : 'var(--section-deep)'
...
const StatTile = ({ stat, subColor }: { stat: StatsBarItem; subColor: string }) => {
    ...
    return (
        <div
            ...
            <div
                className="mt-2 text-[11px] md:text-[12px] font-bold uppercase tracking-[0.16em]"
